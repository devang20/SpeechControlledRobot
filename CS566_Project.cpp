...
{
	char name[100];
	int u_i = 1 , d_i = 1, l_i = 1, r_i = 1, one_i = 1, two_i = 1, three_i = 1;
	int ans;
	...
	do
	{
		...
		else if(word_no == 2)
		{
			...
			sprintf(txtfileName, "LiveTrain/%s_D%d.txt", name, d_i);
			sprintf(wavfileName, "LiveTrain/%s_D%d.wav", name, d_i);
		...
		else if(word_no == 3)
		{
			...
			sprintf(txtfileName, "LiveTrain/%s_L%d.txt", name, l_i);
			sprintf(wavfileName, "LiveTrain/%s_L%d.wav", name, l_i);
		...
		else if(word_no == 4)
		{
			...
			sprintf(txtfileName, "LiveTrain/%s_R%d.txt", name, r_i);
			sprintf(wavfileName, "LiveTrain/%s_R%d.wav", name, r_i);
		...
		else if(word_no == 5)
		{
			...
			sprintf(txtfileName, "LiveTrain/%s_1%d.txt", name, one_i);
			sprintf(wavfileName, "LiveTrain/%s_1%d.wav", name, one_i);

			sprintf(command, "Recording_Module.exe 3 %s %s", wavfileName, txtfileName);

			one_i++;

			sprintf(fileA, "FM/1/A.txt");
			sprintf(fileB, "FM/1/B.txt");
		}
		else if(word_no == 6)
		{
			printf("\nSay \"Two\"\n\n");

			sprintf(txtfileName, "LiveTrain/%s_2%d.txt", name, two_i);
			sprintf(wavfileName, "LiveTrain/%s_2%d.wav", name, two_i);

			sprintf(command, "Recording_Module.exe 3 %s %s", wavfileName, txtfileName);

			two_i++;

			sprintf(fileA, "FM/2/A.txt");
			sprintf(fileB, "FM/2/B.txt");
		}
		else if(word_no == 7)
		{
			printf("\nSay \"Three\"\n\n");

			sprintf(txtfileName, "LiveTrain/%s_3%d.txt", name, three_i);
			sprintf(wavfileName, "LiveTrain/%s_3%d.wav", name, three_i);

			sprintf(command, "Recording_Module.exe 3 %s %s", wavfileName, txtfileName);

			three_i++;

			sprintf(fileA, "FM/3/A.txt");
			sprintf(fileB, "FM/3/B.txt");
		}
		else
		{
		...
		if(flag == 0)
		{
			...
			RiAiCi(); // Calculate Ri, Ai and Ci 
			if(word_no == 1 || word_no == 2 || word_no == 3 || word_no == 4) tokhuraDistance();
			else tokhuraDistanceDigit();
...
void game()
{
	...
	enum MazeObject { HALL, WALL, COIN, BORDER , GATE };
	...
	for (int y = 0; y < HEIGHT; y++) //line enumeration
	{
		...
		{
			maze[y][x] = rand() % 3; // 3 types of objects in the game
